...
public class DatabaseHelper {
    ...
    void initDatastore() {
        ...
        if (HEROKU_MLAB_URI != null && !HEROKU_MLAB_URI.isEmpty()) {
            ...
            datastore = morphia.createDatastore(mongoClient, Constants.Database.HEROKU_DB_NAME);
        } else {
//                //local environ 
            logger.info("Local Database Detected");
            mongoClient = new MongoClient(Constants.Database.HOST, Constants.Database.PORT);
            datastore = morphia.createDatastore(mongoClient, Constants.Database.LOCAL_DBNAME);
        }
