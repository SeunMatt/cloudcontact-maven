...
public class ContactController {
    ...
    public static ModelAndView serveDashboard(Request req, Response res) {
        ...
        String userId = req.session(false).attribute(Constants.ATTR_USER_ID).toString();
        String username = req.session(false).attribute(Constants.ATTR_USER_NAME).toString();
        String email = req.session(false).attribute(Constants.ATTR_EMAIL).toString();
...
//                String username = "Smatt";
        ...
        if (userId != null && !userId.isEmpty() && username != null && !username.isEmpty()) {
            ...
            ModelAndView mv = new ModelAndView(model, Constants.Templates.DASHBOARD);
...
//                logger.info("userId = " + userId + "\nusername = " + username + "\nmodel returned = " + mv.getModel().toString());
            ...
            res.header("Location", Routes.GET_LOGIN_PAGE);
            res.redirect(Routes.GET_LOGIN_PAGE);
            return null;
        }
    }
    ...
    public static int handleNewContact(Request req, Response res) {
        ...
        String data = Jsoup.parse(req.body()).text()
                .replaceAll(Constants.OK_PATTERN, "");
        ...
        try {
            ...
            String userIdNew = req.session(false).attribute(Constants.ATTR_USER_ID);
...
//                logger.info("error parsing data from handleNewContact \n" + e.toString());
    ...
    public static Object handleDeleteContact(Request req, Response res) {
        ...
        String id = Jsoup.parse(req.params("id")).text()
                .replace(Constants.OK_PATTERN, ""); //id of contact to be deleted

        String userId = req.session(false).attribute(Constants.ATTR_USER_ID);
        ...
        if (id != null && !id.isEmpty() && userId != null && !userId.isEmpty()) {
            ...
            if (c != null) {
                ...
                return new Message(Constants.Reply.CONTACT_NOT_FOUND, Constants.Reply.CONTACT_NOT_FOUND_MSG);
            ...
            res.status(500);
            return new Message(Constants.Reply.OK, Constants.Reply.OK_MSG);
        }
    }
